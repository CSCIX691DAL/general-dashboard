...
export class UserhomeComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.userService.getUserGeneratedReportsByUserId().then((reports) => {
      ...
      this.activeUserGeneratedReports = reports.filter(
        (report) => report.isActive === 1
      );
  ...
  private generateUserReport(reportId: string): void {
    this.userService.executeUserGeneratedReport(reportId).then((data) => {
      const widget = this.chartFactory.processChartType(
        this.selectedChartType,
        data,
  ...
  private executeReports(reports: any[]): void {
    reports.forEach((report) => {
      this.userService
        .executeUserGeneratedReport(report.report_id_fk + '')
        .then((data) => {
  ...
  async deleteWidgets(report: UserGeneratedReport): Promise<void> {
    await this.userService
      .deleteUserReport(report.report_id_fk)
  ...
  public setReportInactive(selectedReport: UserGeneratedReport): void {
    this.userService
      .updateIsActive(selectedReport.id, 0)
  ...
  onModelChange(): void {
    ...
    this.seq
      .getModel(this.selectedDatabase.id, this.selectedModel)
      .then((data) => {
  ...
  submit(): void {
    ...
    if (this.selectedFunc === 'Distinct') {
      sql += this.selectColumns().replace(
        this.selectedForFunc,
        'DISTINCT ' + this.selectedForFunc
      );
